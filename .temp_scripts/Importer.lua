	...
	for _, chunkModule in ipairs(jData:GetChildren()) do
		...
		for _, node in ipairs(chunkData) do
			...
			if node.Degree > 2 and not node.IsBridge and not node.IsTunnel then
				...
				if node.JunctionType == "TJunction" then
					addCameraPoint("TJunction", basePart.Position + Vector3.new(0, 15, 0))
				elseif node.JunctionType == "CrossJunction" then
					addCameraPoint("CrossJunction", basePart.Position + Vector3.new(0, 15, 0))
				elseif node.JunctionType == "ComplexJunction" then
					addCameraPoint("Intersection", basePart.Position + Vector3.new(0, 15, 0))
				end
			end
		end
	end
				...
				if #coords >= 2 then
					for i = 1, #coords - 1 do
						...
						if dist > 0.1 then
							...
							part.CFrame = CFrame.new(p1.X, layerY + height / 2, p1.Z)
								* lookAt.Rotation
...
local function buildPolygon(feature, tileFolders)
	local coords = feature.Geometry
	if not coords or #coords < 3 then
		return false
	end
		...
		for _, feature in ipairs(chunkData) do
			...
			if success then
				...
				if feature.OBB then
					...
					if w > stats.MaxBuildingExtentStuds then
						stats.MaxBuildingExtentStuds = w
					end
					if d > stats.MaxBuildingExtentStuds then
						stats.MaxBuildingExtentStuds = d
					end
...
for _ in pairs(stats.UniqueRoundedXCenters) do
	uniqueXCount += 1
end
local uniqueZCount = 0
for _ in pairs(stats.UniqueRoundedZCenters) do
	uniqueZCount += 1
end
...
if stats.MaxBuildingExtentStuds > 2000 then
	warn(
		string.format(
			"Sanity Check Failed: Maximum building extent is %.1f Studs. Coordinate mapping is likely broken.",
			stats.MaxBuildingExtentStuds
		)
	)
...
if #spawnCandidates > 0 then
	...
	for _, c in ipairs(spawnCandidates) do
		if
			(c - Vector3.new(centerX, 0, centerZ)).Magnitude < (bestSpawn - Vector3.new(centerX, 0, centerZ)).Magnitude
		then
			bestSpawn = c
		end
	end
