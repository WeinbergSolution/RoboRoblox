...
createButton("[CENTER]", function()
	tpToCameraPoint("CenterOverview")
end)
createButton("[MAIN ROAD]", function()
	tpToCameraPoint("MainRoad")
end)
createButton("[RESIDENTIAL]", function()
	tpToCameraPoint("ResidentialArea")
end)
createButton("[RAIL AREA]", function()
	tpToCameraPoint("RailArea")
end)
createButton("[POI AREA]", function()
	tpToCameraPoint("POIArea")
end)
		...
		task.spawn(function()
			while task.wait(0.5) do
				...
				if rParts and bParts then
					importStatsLabel2.Text = string.format(
						" Roads: %d / Bldgs: %d / POIs: %d",
						rParts.Value,
						bParts.Value,
						pParts and pParts.Value or 0
					)
				end
