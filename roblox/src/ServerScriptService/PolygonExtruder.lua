...
local function isEar(pts, i)
    ...
    if cross >= 0 then return false end -- Reflex or colinear
...
local function triangulate(vertices)
    ...
    if sum > 0 then
        -- Reverse
        local rev = {}
        for i = n, 1, -1 do table.insert(rev, pts[i]) end
        pts = rev
    end
...
local function createWedge(a, b, c, h, color, material, parent)
    ...
    table.sort(edges, function(e1, e2) return e1[4] > e2[4] end)
    ...
    if altitude < 0.05 then return end -- Degenerate
    
    local model = Instance.new("Model")
    
    local w1 = Instance.new("WedgePart")
    w1.Size = Vector3.new(0.05, altitude, projLength)
    w1.CFrame = CFrame.lookAt(projPoint, p1, Vector3.new(0, 1, 0)) * CFrame.new(0, -altitude/2, -projLength/2) * CFrame.Angles(0, math.pi/2, 0)
    w1.Color = color
    ...
    w2.CFrame = CFrame.lookAt(projPoint, p2, Vector3.new(0, 1, 0)) * CFrame.new(0, -altitude/2, -len2/2) * CFrame.Angles(0, math.pi/2, 0)
    w2.Color = color
