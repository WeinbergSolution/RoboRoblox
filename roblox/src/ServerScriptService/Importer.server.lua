...
if oldGeometry then oldGeometry:Destroy() end
...
local function importRoadsAndRailAndWater()
                ...
                if #coords >= 2 then
                    for i = 1, #coords - 1 do
                        ...
                        if dist > 0.1 then
                            ...
                            if category == "Rail" then cameraPoints.RailArea = p1 + Vector3.new(0,10,0) end
                        end
                    end
                end
            end
    ...
    importSimpleLines("Water", "Water", Config.Colors.Water, Config.Materials.Water, Config.Layers.Water, 0.1, "WaterParts")
end
...
local function importBuildings()
    ...
    for _, chunkModule in ipairs(bData:GetChildren()) do
        ...
        for _, feature in ipairs(chunkData) do
            ...
            if bType == "residential" or bType == "apartments" then color = Color3.fromRGB(180, 170, 160)
            elseif bType == "commercial" or bType == "retail" then color = Color3.fromRGB(150, 180, 200)
            elseif bType == "industrial" then color = Color3.fromRGB(130, 130, 130)
            elseif bType == "school" or bType == "hospital" then color = Color3.fromRGB(220, 220, 200)
            end
            
            local success = false
            if #pts >= 3 then
                local model, isComplex = PolygonExtruder.extrude(pts, hStuds, 0, color, Enum.Material.SmoothPlastic, tileFolders.Buildings, feature.Id)
                if model then
                    success = true
                    trackPart("BuildingParts", #model:GetChildren())
                    if isComplex then stats.BuildingOBBFallbacks += 1 end
                end
            end
            ...
            if not success and feature.OBB then
                ...
                part.CFrame = CFrame.new(cx * scale, hStuds/2, cz * scale) * CFrame.Angles(0, math.rad(-feature.OBB.RotationDegrees), 0)
                part.Color = color
    ...
    for _, chunkModule in ipairs(poiData:GetChildren()) do
        ...
        for _, feature in ipairs(chunkData) do
            ...
            if feature.Type == "Point" then
                ...
                local typ = feature.Properties.amenity or feature.Properties.shop or feature.Properties.public_transport or feature.Properties.railway or "POI"
        ...
        for _, feature in ipairs(chunkData) do
            ...
            if feature.Type == "Polygon" then
                ...
                if feature.OBB then
                    ...
                    part.CFrame = CFrame.new(cx * scale, Config.Layers.Green, cz * scale) * CFrame.Angles(0, math.rad(-feature.OBB.RotationDegrees), 0)
